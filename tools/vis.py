...
#		cax = ax.pcolormesh(array[:-1, :-1, 0], vmin=-1, vmax=1, cmap='jet')
		...
		im = plt.imshow(array[:, :, 0],vmin=Vmin,vmax=Vmax,interpolation='lanczos',aspect='auto')
		fig.colorbar(im)
		def animate(i):
...
def poisson2d(q,gx,gz):
	...
	ksq=(kx**2 + kz**2)
	ftq=fftn(q,axes=ax)
	phi=np.fft.fftn(fftshift((fftshift(ftq,axes=ax)/-ksq[...,np.newaxis]),axes=ax),axes=ax)
	return np.real(phi)
